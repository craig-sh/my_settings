...
**Note:** For `virtnix` and `beelink`, home-manager is embedded in the NixOS config — there is no standalone `homeConfigurations` entry for these hosts. Only test the OS build for them.
...
# Skip for virtnix and beelink (no standalone homeConfigurations):
